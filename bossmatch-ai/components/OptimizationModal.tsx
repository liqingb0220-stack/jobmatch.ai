...
export const OptimizationModal: React.FC<OptimizationModalProps> = ({ job, resumeText, onClose }) => {
  ...
  const handleRunDiagnosis = async () => {
    ...
    try {
      ...
      setProcessStatus(prev => prev.map(s => s.id === 'read' ? {...s, status: 'completed', subText: `✔ 已读取简历（共计识别约 ${Math.floor(resumeText.length / 100)} 个描述片段）`} : s.id === 'jd' ? {...s, status: 'loading', subText: `正在定位 "${job.title}" 的核心能力要求...`} : s));
      
      await new Promise(r => setTimeout(r, 800));
      const result = await getOptimizationDiagnosis(resumeText, job);
      
      setProcessStatus(prev => prev.map(s => s.id === 'jd' ? {...s, status: 'completed', subText: `✔ 已解析 JD（识别多项核心能力维度）`} : s.id === 'mapping' ? {...s, status: 'loading', subText: `🔍 正在匹配你的经历 -> 寻找强化点...`} : s));

      await new Promise(r => setTimeout(r, 600));
      setDiagnosis(result);
      setProcessStatus(prev => prev.map(s => s.id === 'mapping' ? {...s, status: 'completed', subText: `✔ 匹配完毕：发现 ${result.coreGaps.length} 个可强化能力点`} : s));
    } catch (e) {
      setError("诊断生成失败");
    } finally {
